# 規則來源與可信度分類

最後核對：2026-08-02

## 一、來源原則

本專案不將不同性質的資料混為一談。每條規則至少標記下列一類：

| 類型 | 代碼 | 可用範圍 |
| --- | --- | --- |
| 官方 | `official` | 身分證格式、字母區碼與檢查碼等可核對規則 |
| 使用者教材 | `uploaded_material` | 本次提供的生命靈數、九宮格、磁場與部分流年規格 |
| 舊專案 | `legacy_project` | 舊站回歸及相容，不表示較權威 |
| 近代流傳 | `common_practice` | 多版本並存的民俗做法，必須可選 |
| 未解決 | `unresolved` | 資料不足，禁止自動計算或自動解讀 |

「民俗規則被完整照表實作」只代表程式可重現，不代表規則有科學實證或是唯一流派。

## 二、來源 profile

### `taiwan-identity-document-official`

用途只限：

- 台灣國民身分證統一編號格式；
- 新式外來人口統一證號的 8／9 格式；
- 字母區碼；
- 檢查碼邏輯。

參考：

- [政府資料標準平臺：國民身分證統一編號](https://schema.gov.tw/lists/167)
- [行政院公報：國民身分證統一編號檢查方法](https://gazette.nat.gov.tw/EG_FileManager/eguploadpub/eg011228/ch04/type2/gov30/num2/OEg.pdf)
- [內政部移民署：新式外來人口統一證號](https://www.immigration.gov.tw/5385/12162/238449//CP_QA?alias=QAClass&type=%E5%85%A7%E6%94%BF%E9%83%A8%E7%A7%BB%E6%B0%91%E7%BD%B2)
- [財政部：新舊式外來人口統一證號格式差異](https://www.etax.nat.gov.tw/etwmain/tax-info/understanding/tax-q-and-a/national/individual-income-tax/alien-tax-question/other/781zJ3Y)
- [內政部移民署：新式統號比照國民身分證編碼原則](https://servicestation.immigration.gov.tw/1361/1365/6484/6517/246559/)

驗證結果固定標示 `validationScope = format_checksum_only` 與 `issuanceVerified = false`；通過格式與檢查碼不代表證號已配發，也不驗證持有人身分。舊式外來人口統一證號只辨識格式，本版不冒充已實作其官方檢查規則。這些官方資料**不支持** A=01 民俗轉換、八大磁場、人格判讀或人生階段預測。

`taiwan-national-id-official` 保留為舊版相容 profile；新版證號驗證一律回傳上述通用 profile，避免把外來人口證號誤標成國民身分證規則。

### `uploaded-numerology-v2`

來源為使用者於本次任務提供的專案規格及民俗教材整理，涵蓋：

- 完整生日數字加總；
- 主數模式；
- 九宮格模式與連線；
- 1～9 人格資料；
- 八大磁場 64 組；
- 八大磁場中性解讀；
- A=01～Z=26；
- 0／5 literal、bridge_modifier；
- 教材所列人生階段區間；
- 已明確列出的磁場組合關係。

可信度標記為 `folklore`。原始作者、古籍或學術研究來源未獲確認，因此網站不得冠以「古法唯一正統」、「科學證實」或類似宣稱。

### `uploaded-numerology-photo-2026-08-12-v1`

來源為使用者提供的照片 `S__15630383.jpg`，SHA-256：

`9CDD0480522D6AD08121F30662420588533F95F12191CFF78D301C59EDE7B761`

照片補充部分數字的教材代稱、直觀短義、部分常用語與 13 組數字連線。6、8、9 的短代稱以及照片沒有提供的觀察句均明確標成本站摘要／自我觀察句，不冒充照片原文。演算仍沿用 `uploaded-numerology-v2` 的完整生日八位數加總與既有連線判定，沒有另造第二套公式。顯示層作以下安全轉譯：

- 「亞當／夏娃」只標示為原教材代稱，不作性別本質判定；
- 身體部位只列民俗類象並附非診斷聲明，不顯示「有某疾病」；
- 財富、升遷、上司喜愛或宗教眷顧均不作保證；
- `2-4-6-8` 只顯示可能累積情緒壓力，明示不是心理或醫療診斷；
- 「吃菜命」定義不足，列為未解決原句，不納入結果。

結果頁會把目前算出的 1～9 基底固定顯示在「數理結果」右欄；手機改為緊接在結果下方。內容分成核心特質、互動方式、工作觀察、需要留意、原教材常用語與來源界線。照片中語意不足或涉及醫療資格、疾病、財富、升遷、宗教眷顧、人際受寵與命定保證的句子，只能顯示其安全轉譯或不採用原因，不得改寫成事實判定。

2026-08-12 逐字複核修正：數字 4 原文為「很專情」，不是「專業」；數字 5 的「有美食」語意不足；數字 6 的「心裡治療師」只轉譯為常成為朋友傾訴的對象，不代表心理師資格或治療能力。

原始作者、出版資訊與學術依據未獲確認，可信度為 `folklore`。

### `legacy-project-v1`

來源為本專案既有 `calculator-core.js` 行為，涵蓋：

- 月、日、年分段化簡的生命路徑；
- 保留 11、22、33；
- 原始生日數字計數；
- 洛書版位顯示；
- 生日個人流年既有公式；
- 舊版人格與色彩文案。

用途只限回歸與舊結果相容。舊站沒有八大磁場、身分證流年與分析歷史資料，不能為相容目的補造舊結果。

### `timeline-common-practice-v1`

涵蓋：

- 首段 13 年、其後 5 年；
- 5 年循環延伸。

這些是近代流傳版本，與教材原表及首段 10 年版本互有差異。必須由使用者在設定中選擇，結果頁需顯示目前 profile。

### `identity-destiny-common-practice-v1`

涵蓋身分證命格數列與人生階段數列的分流：

- 英文字母依 A=01～Z=26 轉換；
- 命格分析時，01～09 只移除第一個補位 0；
- J=10～Z=26 不移除任何字母碼數字；
- 人生階段保留完整兩位字母碼；
- 兩種數列都使用相鄰滑動配對。

交叉參考：

- [生命靈數 DNA 數字解析法](https://www.fox-saying.com/blog/posts/41944909)
- [易經生命零數整理](https://chan-lead.blogspot.com/p/blog-page_24.html)
- [數字易經看身分證字號教學](https://star-artnlp.com/how-to-read-the-digital-i-ching-id-number-learn-fortune-telling-transit-time-and-numerology-online-in-one-step/)

三者屬近代網路流傳整理，不是政府標準、古籍定論或科學證據，因此可信度仍標記為 `folklore`。

## 三、規則與來源對照

| 規則 | 來源 profile | 可信度 | 備註 |
| --- | --- | --- | --- |
| 台灣證號格式／檢查碼 | `taiwan-identity-document-official` | 官方邏輯 | 不含民俗解讀，也不查證配發狀態 |
| 完整生日八位數加總 | `uploaded-numerology-v2` | 民俗教材 | 新版預設 |
| 分段月日年加總 | `legacy-project-v1` | 舊站行為 | 相容模式 |
| 生日九宮格與連線 | `uploaded-numerology-v2` | 民俗教材 | 0 不入格 |
| 洛書版位 | `legacy-project-v1` | 舊站行為 | 僅相容顯示 |
| 1～9 人格 | `uploaded-numerology-v2` | 民俗教材 | 不作心理診斷 |
| 1～9 直觀代稱、短義與口頭標記 | `uploaded-numerology-photo-2026-08-12-v1` | 使用者照片／民俗教材 | 疾病、財富、升遷、宗教斷言已排除 |
| 八大磁場 64 組 | `uploaded-numerology-v2` | 民俗教材 | 完整性可測，真實性非科學結論 |
| A=01～Z=26 | `uploaded-numerology-v2` | 民俗教材 | 與官方區碼分離 |
| 0／5 橋接 | `uploaded-numerology-v2` | 民俗教材 | 保留原組與修飾鏈 |
| 首段 10 年後每 5 年 | `uploaded-numerology-v2` | 民俗教材 | 新版預設，畫面需明示 |
| 首段 13 年後每 5 年 | `timeline-common-practice-v1` | 近代流傳 | 可選 |
| 五年循環 | `timeline-common-practice-v1` | 近代流傳 | 延伸區間標示輪次 |
| 身分證命格數列 | `identity-destiny-common-practice-v1` | 近代流傳 | 01～09 只移除字母碼第一個補位 0 |
| 健康部位對應 | `uploaded-numerology-v2` | 民俗教材 | 無醫療效力 |

## 四、爭議與未解決規則

1. 人生階段區間存在首段 10 年、13 年與循環等版本，沒有足夠證據判定唯一正確。
2. 教材原表有 11 個區間，但標準身分證 A=01 轉換後有 11 位數、只形成 10 組相鄰配對；系統保留不一致警告。
3. 「前天、絕、後生」等簡寫未具完整前後文與匹配定義，規則停用。
4. 「命格數」在不同系統可能指姓名表達數、生日生命靈數或身分證格局；本站只採明確標示的「身分證命格數列」定義，不混用其他系統。
5. 磁場、人格、財富、感情、健康等描述均不得提升為科學、醫療、投資或風險預測。

## 五、文案限制

不得使用：

- 必然發財、必然破財；
- 必然生病、可以治療；
- 一定發生事故；
- 官方認證磁場；
- 古籍證明（未有可查來源時）；
- 唯一正統算法。

可使用：

- 「依本次教材規則」
- 「此民俗分類可中性理解為」
- 「此版本的計算結果」
- 「流派版本不同，請於設定確認」
- 「此為民俗文化娛樂，不是科學或專業判斷」

## 六、健康與安全免責

任何健康相關教材註記都需附：

> 此為原教材中的民俗對應，無醫學診斷效力；身體不適應諮詢合格醫療專業人員。

任何完整報告都需附：

> 本分析僅供民俗文化、娛樂及自我觀察，不構成醫療、心理、法律、投資或人生決策建議。
