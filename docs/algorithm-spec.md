...
- `title`
- `teachingLabel`
- `labelAuthority`（`photo`／`editorial`）
- `quickMeaning`
- `positiveTraits`
- `challengeTraits`
- `socialTraits`
- `workTraits`
- `observationPhrase`
- `phraseAuthority`（`photo`／`editorial`）
- `healthFolkloreNotes`
- `unresolvedSourceClaims`
- `disclaimer`
- `sourceProfile`（主要顯示來源）
- `sourceProfiles`（完整來源組合）
...
每一段必須由 `buildTimelineStageInsight()` 產生同一份結構化內容，供畫面與文字報告共用：

- 階段主題：沿用該磁場既有 `core` 關鍵詞；
- 可觀察：把既有關鍵詞改寫成中性自我檢核問題；
- 可運用：沿用既有 `strengths`；
- 需要留意：沿用既有 `cautions`；
- 前段轉接：只比較時間軸中真正相鄰的前一段；
- 分類依據：說明直接分類、0／5 未分類、橋接補充或無配對狀態。

細解不得包含完整身分證、完整轉換數列或未遮罩相鄰組合。

### 13.2 未分類與橋接界線

- 含 0／5 的直接相鄰組合不在 64 組基礎表時，必須維持 `modifier_unclassified`。
- `未分類` 不代表無效、負面或沒有影響。
- 若同一位置確實有橋接結果，只能列為補充磁場，不能取代直接分類。
- 相鄰兩段任一段未分類時，不得跳過它拼接其他已分類階段。
- 沒有已啟用的組合規則時，必須明示「尚無細則」，不得自行補造吉凶或事件。
