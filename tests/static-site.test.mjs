...
test("GitHub Pages entrypoint is numerology-first with three analyzers and a separate Shao Kangjie option", async () => {
  ...
  assert.match(appSource, /from "\.\/site-services\.js\?v=20260812-page-views-v1"/);
  ...
  assert.match(reactSource, /data-active-mode=\{mode\}/);
  assert.match(appSource, /function createYaoLegend\(\)/);
  assert.match(reactSource, /function YaoLegend\(\)/);
  assert.match(appSource, /function createHexagramLines\(lines, texts/);
  assert.match(appSource, /hexagram-judgment/);
  assert.match(appSource, /line-text/);
  assert.match(reactSource, /texts: ReturnType<typeof getIChingText>\["lines"\]/);
  assert.match(reactSource, /hexagram-judgment/);
  assert.match(reactSource, /line-text/);
...
test("Shao Kangjie static page keeps every primary title in an independent brush asset and exposes all supported derivations", async () => {
  ...
  assert.match(scriptSource, /function createHexagramLines\(lines, texts/);
  assert.match(scriptSource, /hexagram-judgment/);
  assert.match(scriptSource, /line-text/);
  assert.match(appSource, /texts: IChingLineText\[\]/);
  assert.match(appSource, /hexagram-judgment/);
  assert.match(appSource, /line-text/);
  assert.match(styles, /--yao-yang: #e86762/);
  assert.match(styles, /--yao-yin: #5aa9df/);
  ...
  assert.match(styles, /\.hexagram-card > \.hexagram-judgment/);
  assert.match(styles, /\.line-row > \.line-text/);
