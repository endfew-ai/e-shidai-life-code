...
function BrushTitle({ src, text, className = "", width, height, eager = false }: { src: string; text: string; className?: string; width?: number; height?: number; eager?: boolean }) {
  return <span className={`brush-title ${className}`.trim()}><span className="sr-only">{text}</span><img className="brush-title-image" src={src} width={width} height={height} loading={eager ? undefined : "lazy"} fetchPriority={eager ? "high" : undefined} decoding="async" alt="" aria-hidden="true" /></span>;
}
...
function writeDetectedFields(form: HTMLFormElement | null, detected: CurrentCalendarParts) {
  ...
  const leapField = form.elements.namedItem("isLeapMonth");
  if (leapField instanceof HTMLInputElement && leapField.type === "checkbox") leapField.checked = detected.isLeapMonth;
}
...
function AccessGate({ onUnlock }: { onUnlock: () => void }) {
  ...
  return <section className="access-gate" aria-labelledby="access-gate-title"><img className="access-gate-art" src="/visuals/ai-dashboard/kangjie-study-v1.webp" width={960} height={640} fetchPriority="high" decoding="async" alt="" aria-hidden="true" /><div className="access-gate-card"><span className="access-gate-index">PRIVATE ACCESS・專頁存取</span><h1 id="access-gate-title"><BrushTitle src="/visuals/brush/title-kangjie-entry-web-v1.webp" text="邵康節易學" className="brush-access-gate" width={600} height={154} eager /></h1><p>輸入四位密碼，驗證後進入年月日時與梅花易數衍算。</p><form onSubmit={handleSubmit} noValidate><label htmlFor="kangjie-password-react">存取密碼</label><div><input ref={inputRef} id="kangjie-password-react" name="password" type="password" inputMode="numeric" autoComplete="current-password" maxLength={4} pattern="[0-9]{4}" placeholder="輸入 4 位數字" value={password} onChange={(event) => { setPassword(event.target.value.replace(/\D/g, "").slice(0, 4)); setMessage(""); }} required /><button type="submit">驗證後進入</button></div><p data-access-message role="alert" aria-live="polite">{message}</p></form><Link href="/">返回首頁</Link></div></section>;
}
...
function KangjieResult({ result }: { result: KangjieAnalysis }) {
  ...
  return <section className="kangjie-calculation-result" aria-labelledby="react-kangjie-result-title">
    ...
    {result.seasonalStrength && <div className="seasonal-ledger"><header><strong>月令旺衰</strong><span>農曆 {result.seasonalStrength.lunarMonth} 月；只標旺、衰、平</span></header>{[["體卦", result.seasonalStrength.body], ["用卦", result.seasonalStrength.use], ["下互", result.seasonalStrength.mutualLower], ["上互", result.seasonalStrength.mutualUpper], ["變卦用方", result.seasonalStrength.transformedUse]].map(([label, item]) => { const strength = item as typeof result.seasonalStrength.body; return <article key={String(label)}><span>{label as string}</span><strong>{strength.trigramName}・{strength.status}</strong><small>{strength.season}・{strength.monthBranch}月</small></article>; })}<p>{result.seasonalStrength.note}</p></div>}
...
function HuangjiResult({ result }: { result: HuangjiAnalysis | HuangjiPositionAnalysis }) {
  const trace = result.calculationTrace as {
  ...
  const values = result.mode === "duration"
  ...
  return <section className="huangji-calculation-result"><p className="section-index">{result.mode === "duration" ? `時間長度 ${result.totalYears} 年` : `${result.targetLabel}・${result.profileLabel}`}</p><h2 tabIndex={-1}><BrushTitle src="/visuals/brush/title-kangjie-result-v1.webp" text="衍算結果" className="brush-kangjie-result" /></h2><div className="huangji-output-grid">{values.map(([label, value]) => <article key={label}><span>{label}</span><strong>{value}</strong></article>)}</div><code className="huangji-equation">{result.equation}</code><details className="calculation-audit huangji-audit"><summary><strong>完整演算明細</strong><span>{result.algorithmVersion}・{result.profileId}</span></summary><div className="calculation-audit-body"><div className="calculation-input-grid"><article><span>原始輸入</span><pre>{JSON.stringify(trace.originalInput, null, 2)}</pre></article><article><span>正規化輸入</span><pre>{JSON.stringify(trace.normalizedInput, null, 2)}</pre></article></div><ol className="huangji-step-list">{(trace.steps || []).map((step, index) => <li key={index}><code>{JSON.stringify(step)}</code></li>)}</ol><ul className="calculation-warning-list">{(trace.warnings || []).map((warning) => <li key={warning}>{warning}</li>)}</ul><div className="calculation-source-list">{result.sourceRefs.map((source) => <a href={source.url} target="_blank" rel="noreferrer" key={source.id}><strong>{source.title}</strong><span>{source.organization}・{source.id}</span></a>)}</div></div></details><p className="iching-boundary">{result.mode === "duration" ? "這是時間長度的單位換算，不是西元紀年定位、天文週期證明或事件預言。" : `${result.epoch.notice} 本結果依可設定錨點作數學定位。`}</p></section>;
}
...
export default function KangjiePage() {
  ...
  const [objectTrigram, setObjectTrigram] = useState("1");
  const [objectCandidateConfirmed, setObjectCandidateConfirmed] = useState(false);
  ...
  function handleManualTimeChange(event: FormEvent<HTMLFormElement>) {
    ...
    if (!["yearBranch", "lunarMonth", "lunarDay", "hourBranch", "isLeapMonth"].includes(target.name)) return;
    const wasManual = manualTimeOverrideRef.current;
    const snapshot: Record<string, string> = {};
    if (!wasManual) {
      for (const name of ["yearBranch", "lunarMonth", "lunarDay", "hourBranch", "isLeapMonth"]) {
        const field = event.currentTarget.elements.namedItem(name);
        if (field instanceof HTMLInputElement && field.type === "checkbox") snapshot[name] = field.checked ? "on" : "";
        else if (field instanceof HTMLInputElement || field instanceof HTMLSelectElement) snapshot[name] = field.value;
      }
    }
    setManualTimeValues((previous) => ({ ...(wasManual ? previous : snapshot), [target.name]: target.type === "checkbox" ? (target.checked ? "on" : "") : target.value }));
  ...
  async function handleMeihua(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (method === "calendar") {
        const calendarInput = !manualTimeOverride && currentTime
          ? { ...currentTime, ...values, mode: "automatic", calendarProfile }
          : { ...values, mode: "manual", calendarProfile, timeZone: calendarTimeZone };
        next = calculateCalendarHexagram(calendarInput, { profile: activeProfile });
  ...
  const objectCandidates = findObjectTrigramCandidates(objectDescription);
  ...
  return <>{!accessGranted && <AccessGate onUnlock={unlockAccess} />}<main ref={protectedContentRef} className={`site-shell kangjie-shell${accessGranted ? "" : " is-access-locked"}`} data-protected-content aria-hidden={accessGranted ? undefined : true}>
    ...
    <section className="kangjie-workspace" id="workspace" aria-label="邵康節易學內容與衍算">
      ...
      {pageTab === "meihua" && <section className="kangjie-panel" role="tabpanel">
        ...
        <div className="method-forms">
          ...
          <form ref={meihuaFormRef} className="kangjie-form" onSubmit={handleMeihua} onChange={handleManualTimeChange} noValidate>
            ...
            {method === "supplement" && <><label className="field-block"><span>補充方法<small>每種方法各守其法</small></span><select name="supplementType" value={supplementType} onChange={(event) => setSupplementType(event.target.value)}><option value="zhang-chi">丈尺占</option><option value="chi-cun">尺寸占</option><option value="posterior">後天端法</option><option value="person">為人占</option><option value="self">自己占</option><option value="animal">動物占</option><option value="static">靜物占</option><option value="direction">方位取卦</option></select></label>{["zhang-chi", "chi-cun"].includes(supplementType) ? <><div className="triple-input-grid">{supplementType === "zhang-chi" && <label className="field-block"><span>丈數<small>作上卦</small></span><input name="zhang" type="number" min="1" defaultValue="1" /></label>}<label className="field-block"><span>尺數<small>依方法入卦</small></span><input name="chi" type="number" min="1" defaultValue="1" /></label><label className="field-block"><span>寸數<small>丈尺占不採用</small></span><input name="cun" type="number" min="1" defaultValue="1" /></label></div>{supplementType === "chi-cun" && <><label className="field-block"><span>尺寸版本<small>可核傳本加時；另保留未證異法</small></span><select name="version"><option value="modern-with-hour">傳本主法：動爻加時辰</option><option value="old-without-hour">未證異法：動爻不加時辰</option></select></label><BranchSelect name="hourBranch" label="時支" defaultValue={timeDefaultValue("hourBranch", currentTime?.hourBranch)} /></>}</> : <><label className="field-block"><span>所見物象描述<small>只列候選，必須由你確認</small></span><input name="objectDescription" value={objectDescription} onChange={(event) => { setObjectDescription(event.target.value); setObjectCandidateConfirmed(false); }} placeholder="例如：紅花、圓鏡、枯枝" /></label>{objectCandidates.length > 0 && <div className="object-candidate-list" aria-label="物象卦候選">{objectCandidates.map((candidate) => <button type="button" className={objectTrigram === String(candidate.trigramId) ? "is-selected" : ""} onClick={() => { setObjectTrigram(String(candidate.trigramId)); setObjectCandidateConfirmed(true); }} key={candidate.trigramId}><strong>{candidate.trigram}{candidate.trigramId}</strong><small>{candidate.matches.join("、")}</small></button>)}</div>}<input name="objectCandidateConfirmed" type="hidden" value={objectCandidateConfirmed ? "true" : "false"} /><div className="triple-input-grid"><label className="field-block"><span>物象卦<small>使用者最終確認</small></span><select name="objectTrigram" value={objectTrigram} onChange={(event) => { setObjectTrigram(event.target.value); setObjectCandidateConfirmed(true); }}>{[["1", "乾1"], ["2", "兌2"], ["3", "離3"], ["4", "震4"], ["5", "巽5"], ["6", "坎6"], ["7", "艮7"], ["8", "坤8"]].map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label className="field-block"><span>方位卦<small>請依方向確認</small></span><select name="directionTrigram">{[["1", "西北・乾1"], ["2", "西・兌2"], ["3", "南・離3"], ["4", "東・震4"], ["5", "東南・巽5"], ["6", "北・坎6"], ["7", "東北・艮7"], ["8", "西南・坤8"]].map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><BranchSelect name="hourBranch" label="時支" defaultValue={timeDefaultValue("hourBranch", currentTime?.hourBranch)} /></div>{["animal", "static"].includes(supplementType) && <label className="field-block"><span>觸發事件<small>{supplementType === "static" ? "初創、置成或異常觸發" : "記錄單一所見之物與來向"}</small></span><input name="trigger" type="text" placeholder="例如：枯枝無風自墜" /></label>}{supplementType === "animal" && <input name="subjectCount" type="hidden" value="1" />}</>}</>}
      ...
      {pageTab === "huangji" && <section className="kangjie-panel" role="tabpanel">
        ...
        <div ref={huangjiRef} className="huangji-result-anchor">{huangji && <HuangjiResult result={huangji} />}</div>
      </section>}

      {pageTab === "sources" && <section className="kangjie-panel" role="tabpanel"><PageHeading index="可回到原文逐條核對" src="/visuals/brush/title-kangjie-source-v1.webp" title="原文與來源" source>古籍、官方曆法與筆畫資料均固定版本並保留來源。</PageHeading><div className="source-ledger">{[["梅花易數・卷一", "卦數、年月日時、物數、聲音、字占、丈尺與後天端法", "https://zh.wikisource.org/zh-hant/梅花易數/卷一"], ["梅花易數・卷二", "體用與五行生剋交叉核對", "https://ctext.org/wiki.pl?chapter=475043&if=gb"], ["維基文庫《周易》", "本站內嵌卦爻本文的實際來源", "https://zh.wikisource.org/zh/周易"], ["皇極經世書", "元會運世結構與固定數制", "https://www.kanripo.org/text/KR3g0006/001"], ["中央氣象署日曆資料", "國農曆與節氣測試基準", "https://opendata.cwa.gov.tw/dataset/all/A-A0087-001"], ["Unicode Unihan 17.0", "kTotalStrokes 本機筆畫索引", "https://www.unicode.org/reports/tr38/"], ["教育部《國語辭典簡編本》", "總筆畫 provider 與公眾授權界線", "https://language.moe.gov.tw/001/Upload/Files/site_content/M0001/respub/dict_concised_download.html"]].map(([title, note, href]) => <a href={href} target="_blank" rel="noreferrer" key={title}><span>{title}</span><strong>{note}</strong><small>開啟來源 ↗</small></a>)}</div><aside className="kangjie-boundary"><span aria-hidden="true">※</span><div><h2><BrushTitle src="/visuals/brush/title-kangjie-boundary-v1.webp" text="推演界線" className="brush-kangjie-boundary" /></h2><p>本工具依現行傳本製作，供傳統文化研究、演算核對與娛樂。它不是科學預測，不主張唯一正統，也不作醫療、法律、投資、工作、人事或重大人生決策依據。</p></div></aside></section>}
    </section>
